...
class _StatisticsPageState extends State<StatisticsPage> {
  ...
  void initState(){
    ...
    biteListBLoC.getAllBites(); // import and processes raw data into a list of bites
    super.initState();
  }
          ...
          Expanded( 
            ...
            child: ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index){
                return Container( // container to add some padding to make it look nice
                  padding: const EdgeInsets.all(2.0),
                  child: BiteCard(bite: biteListBLoC.biteList[index], isDialog: false,).build(context), // makes a card to display each animes information
                );
              }, 
            )
          ),
...
class BiteCard extends StatelessWidget{
  ...
  Widget build(BuildContext context){
    return Card(
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)), // rounded cards look nice

      // list tile to format information
      child: ListTile(
        leading: Image.asset(bite.imageURL, height: 40, width: 40), // image of bite location
        title: Text('${toMonthName(bite.dateBitten.month)} ${toOrdinal(bite.dateBitten.day)}, ${bite.dateBitten.year}'), // date of bite
  ...
  _showDeleteBiteForm(BuildContext context) async {
    ...
    if(!isDialog){ // if is in dialog box disable button
  ...
  _showUpdateBiteForm(BuildContext context) async {
    ...
    if(!isDialog){ // if called in a dialog box disable button
